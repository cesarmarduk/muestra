...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('dashboard.management.contact.update', Crypt::encrypt($contact->id)) }}">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <div class="input-group @if($errors->get('fullname')) is-invalid @endif">
                                    ...
                                    <input type="text" id="fullname" class="form-control form-control-lg text-white" name="fullname" value="{{ old('fullname', $contact->fullname) }}" placeholder="Ingrese nombres y apellidos" autofocus>
                                </div>
                            </div>
                        </div>                            
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <div class="input-group @if($errors->get('email')) is-invalid @endif">
                                    ...
                                    <input type="email" id="email" class="form-control form-control-lg text-white" name="email" value="{{ old('email', $contact->email) }}" placeholder="Ingrese email">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-2 col-sm-1 col-md-1">
                            <div class="form-group">
                                <label for="">&nbsp;</label>
                                <button class="btn btn-info btn-icon waves-effect wave-dark" data-toggle="modal" data-target="#addressModal" data-backdrop="static" type="button"><i class="material-icons">add</i></button>
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
<script type="text/javascript">
    ...
    let addressByContact = () => {
        let value = '{{ Crypt::encrypt($contact->address_id) }}';
        $.ajax({
            url: "{{ route('dashboard.management.address.editAjax') }}",
            data: {
                val: value
            },
            dataType: "JSON",
            ...
            statusCode: {
                200: function (res) {
                    $("#address_id").html(`<option selected value="${res.id}">${res.address}</option>`);
                }
            },
        });
    }
